      ...
      {loading || !lead ? (
        ...
        <ScrollArea className="flex-1">
          <div className="p-5 space-y-5">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {lead.assigned_user ? (
                    <Badge variant="default" className="text-[11px] py-0 px-1.5 gap-0.5">
                      <UserRound className="w-3.5 h-3.5" /> ASSIGNED
                    </Badge>
                  ) : (
                    <Badge variant="default" className="text-[11px] py-0 px-1.5 gap-0.5 opacity-60">
                      UNASSIGNED
                    </Badge>
                  )}
                </div>
                {lead.full_name && (
                  <p className="text-sm font-mono text-[#666] mt-0.5">{lead.full_name}</p>
                )}
                ...
                <div className="flex items-center gap-1.5 mt-1.5">
                  <Badge variant="source" className="text-[11px] py-0 px-1.5">{lead.source}</Badge>
                  {/* Stage selector */}
                  <div className="relative">
                    <button
                      onClick={() => setStageOpen(!stageOpen)}
                      className="flex items-center gap-1 px-2 py-0.5 bg-[#111] border border-[#222] rounded text-[11px] font-mono text-[#888] hover:text-[#ccc]"
                    >
                      ...
                      <div className="absolute left-0 top-full mt-1 bg-[#111] border border-[#222] rounded shadow-xl z-50 min-w-[130px]">
                        {STAGES.map((s) => (
                          <button
                            ...
                            className={cn(
                              'w-full text-left px-3 py-1.5 text-xs font-mono hover:bg-[#1a1a1a]',
            ...
            {lead.bio && (
              <p className="text-[13px] font-mono text-[#555] italic bg-[#0d0d0d] rounded p-2 border border-[#111]">
                {lead.bio}
              </p>
            )}
            ...
            {activeTab === 'details' ? (
              <div className="space-y-5">
                ...
                {lead.conversation?.summary && (
                  <div className="bg-[#0d0d0d] rounded-lg border border-[#1a1a1a] p-3">
                    <span className="text-xs font-mono text-[#666] uppercase tracking-wider">AI Summary</span>
                    <p className="text-[13px] font-mono text-[#888] leading-relaxed mt-1.5">
                      {lead.conversation.summary}
                    </p>
                ...
                {lead.conversation && (
                  ...
                  >
                    ...
                    <span className="text-[13px] font-mono text-[#888]">View conversation in Inbox</span>
                    <ExternalLink className="w-4 h-4 text-[#444] ml-auto" />
                  </a>
                )}

                {/* 6. Assignment */}
                <div className="bg-[#0d0d0d] rounded-lg border border-[#1a1a1a] p-3">
                  <span className="text-xs font-mono text-[#666] uppercase tracking-wider">Assignment</span>
                  <div className="flex items-center gap-2 mt-1.5">
                    {lead.assigned_user ? (
                      <>
                        ...
                        />
                        <div>
                          <p className="text-[13px] font-mono text-[#ccc]">{lead.assigned_user.name}</p>
                          <p className="text-[11px] font-mono text-[#555]">{lead.assigned_user.role}</p>
                        </div>
                      </>
                    ) : (
                      <span className="text-[13px] font-mono text-[#666]">Unassigned</span>
                    )}
                  </div>
                </div>
                ...
                {/* 7. Notes */}
                <div className="bg-[#0d0d0d] rounded-lg border border-[#1a1a1a] p-3">
                  <span className="text-xs font-mono text-[#666] uppercase tracking-wider">Notes</span>
                  <div className="mt-2 space-y-2">
                    {lead.notes.map((note) => (
                      <div key={note.id} className="text-[13px] font-mono text-[#888] bg-[#111] rounded p-2">
                        {note.content}
                  ...
                  <div className="flex gap-2 mt-2">
                    ...
                    <button
                      ...
                      className="self-end px-3 py-2 bg-[#111] border border-[#222] rounded text-xs font-mono text-[#888] hover:text-[#00ff88] hover:border-[#00ff88]/30 disabled:opacity-50"
                    >
                ...
                {lead.follow_up && (
                  <div className="bg-[#0d0d0d] rounded-lg border border-[#1a1a1a] p-3">
                    <span className="text-xs font-mono text-[#666] uppercase tracking-wider">Follow-Up</span>
                    <div className="mt-1.5">
                      <p className="text-[13px] font-mono text-[#ccc]">{lead.follow_up.sequence.name}</p>
