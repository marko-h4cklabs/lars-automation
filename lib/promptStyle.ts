 ...
 * copies formatting from the prompt and dashes flag as chatbot behavior.
 */
export function buildStyleBlock(rules: StyleRules): string {
  ...
  if (rules.never_use_em_dash) {
    lines.push('* NEVER use dashes of any kind in messages: no hyphen, no en dash, no em dash. Use commas or periods instead.')
  }
  ...
  if (ext(rules).multi_message_bursts) {
    lines.push('* Split thoughts into 2 to 3 short separate messages instead of one long block.')
  }
  ...
  if (ext(rules).vibe_length_matching) {
    lines.push("* MATCH the lead's message LENGTH: if they send 3 words, reply with 3 to 8 words. Short messages get short replies.")
  }
